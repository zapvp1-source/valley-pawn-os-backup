...
STEP 5a — Post the table to #jewlery-counts (C0BM9NHGTT4) — FORMAT SET BY JOSHUA 2026-08-14.
One markdown table, NO commentary, no reasons, no emoji, nothing else except a first line:
"Jewelry Count — <Weekday> <M/D> (expected = system on-hand, counted = PM count sheet)".
Columns: Store | Category | Expected | Counted | Variance. Expected = the Bravo freeze-window
on-hand count (Chains+Necklaces summed into Necklaces). Counted = the PM sheet. Variance =
Counted - Expected, signed (+/-/0). One row per category per store plus a bolded Total row per
store. Open stores only. This is what Joshua wants the field to see — the sold-based/net-change
methodology is retired for channel posts. If ANY store is missing/stale/non-ok, post nothing
(all-or-nothing rule above) and follow the failure path.

STEP 5b — Report to Joshua.
