...
## 2026-08-24 � REAL FIX SHIPPED (Culpeper + Harrisonburg). Waynesboro/Lexington/Roanoke blocked on current passwords.
...
48 hours despite this task firing every 10 minutes the whole time � culpeper alone went from a
cleared-to-near-0 state up to 1,487 unarchived. The 8/22 "refutation" (net counts went down during
one measurement window) was real but not dispositive � it didn't survive a longer observation
window. The single most rigorous piece of evidence in this file � the direct message-ID probe that
found a moved message back in INBOX � was correct: moving to [Gmail]/All Mail over IMAP is not
...
Actual fix implemented: a server-side Gmail filter per store account � from:(ebay.com OR
...
a store address is untouched in store staff's own inbox view � directly addresses the concern the
...
This task (store-mail-archive-sweep) is now fully superseded for Culpeper and Harrisonburg �
...
to near-0, then this whole task can be disabled fleet-wide and archived � its job will be done by
Gmail natively for all 5 stores.

Next action for Joshua/Preston: current Gmail passwords for waynesboro@fcfpawn.com,
lexington@fcfpawn.com, roanoke@fcfpawn.com � reply with them or update the store-credentials
skill directly, and the next session (or this one, resumed) finishes the remaining 3 filters in
under 10 minutes.


## 2026-08-24 (later) � ALL 5 STORES NOW HAVE THE REAL FIX. Task DISABLED (superseded).
...
1. The eBay pattern only matched .com domains � a members.ebay.de / members.ebay.co.uk message
...
2. The original diagnosis was scoped too narrowly to eBay/Amazon notifications. Live sender audit
   ...
   remaining inbox volume � none of these are real customer correspondence, all safe to archive
   the same way.
