...
On Wednesdays only Culpeper traded and only Culpeper will have a fresh sheet tonight. Still pull all 5 stores (the other four should be unchanged from Tuesday, which is itself a useful integrity check), but a Wednesday run is NOT incomplete just because four stores have no new sheet.
...
Tail logs/jewelry-onhand-<YYYY-MM-DD>.log periodically via osascript. Expect roughly 45-60 minutes for 5 stores.
