...
- Tracking spreadsheet: Look for "VP BONUS FINAL Updated.xlsx" or "VP BONUS FINAL*.xlsx" in the "Claude 4 back up" mounted folder (typically at a path like /sessions/<session>/mnt/Claude 4 back up/)
...
The Friday multiplier should not fall below 1.0 — if it would, hold it at 1.0 and flag this for Joshua.
...
Use computer-use tools (Parallels → Chrome) to open Bravo and navigate to the KPI Report for the completed month. Read the bravo-context skill first if you need navigation help. If the report is already visible on screen, scrape it visually.

Extract for EACH store:
- Net Revenue for the completed month
...
Open VP BONUS FINAL Updated.xlsx using openpyxl. Read the "2025 compared to Bonus" sheet.

Spreadsheet layout:
- Culpeper: header row 9, data rows 10-21, current-year April = row 13
- Harrisonburg: header row 24, data rows 25-36, April = row 28
- Roanoke: header row 39, data rows 40-51, April = row 43
- Lexington: header row 54, data rows 55-66, April = row 58
- Waynesboro: header row 69, data rows 70-81, April = row 73
- Company total: header row 84, data rows 85-96, April = row 88

Column mapping (A=1 through H=8):
- A: Month name
- B: 2025 Revenue (prior year)
- C: 2026 Bonus Target
- D: 2026 Revenue Actual
- E: Variance (D - C)
- F: Ending Assets Target
- G: Actual Ending Assets
- H: Yield (D / prior month G)
...
Use openpyxl to update the file. IMPORTANT: be careful with merged cells — use a try/except around each cell write to skip merged non-primary cells gracefully. Never overwrite formula cells (e.g., Company row C column which contains a sum formula).

For the COMPLETED month row in each store block:
- Column D: actual revenue from Bravo
- Column E: variance = D - C (only if E is not a formula; if it's a formula, leave it)
- Column G: actual ending assets from Bravo
- Column H: yield = revenue / prior month ending assets
...
Write mode: load with data_only=False, preserve existing formulas, write updated values, save back.
Save to the same path as the source file (VP BONUS FINAL Updated.xlsx in Claude 4 back up folder).
chmod 0o644 after saving.
...
2. Confirmation that spreadsheet was updated with actuals + new targets
