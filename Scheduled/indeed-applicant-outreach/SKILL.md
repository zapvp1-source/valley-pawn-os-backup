...
STEP 0.7 -- PRESTON EXCLUSION LIST CHECK, before touching any named candidate (added 2026-09-02). HIRING_OUTREACH.md now has a PRESTON ALREADY HANDLED table, right before the Rules section. Check every candidate name against it, and check the Indeed Notes tab for a Preston already-talked-to-them note, BEFORE any send in Step 1/2/4 or any booking in Step 5. A match means do not contact, do not book, delete any existing calendar event for them, and log CLOSED, PRESTON ALREADY HANDLED. This exists because Preston stop-rebooking instructions land in Slack #preston-claude, a channel this task never reads; the exclusion table is the handoff. If Preston flags someone new via any channel you do see this run, add a row to that table yourself.
