...
**STANDING RULE — DATA ONLY in the Slack post.** The operations team reads `#aged-inventory-review`. They do not need source footers, process commentary, or pipeline status notes. Post the title, the ranked table, the one-line performer callout (see below), and stop. No `_Source: Bravo POS · ..._` line, no `_Update: ..._`, no multi-bullet "Key Takeaways" narrative paragraphs.

**RANKING — LOWEST % IS THE BEST PERFORMER.** The table is ranked highest-to-lowest by Total Aged % (so the worst performer is at the top and draws the eye to where action is needed). Add a one-line callout under the table that names the leader and the laggard, in this format:

```
🏆 Cleanest book: [Store] ([Tot%]).  🛠️  Needs the most attention: [Store] ([Tot%]).
```

This is the only narrative line in the post. No other commentary, no multi-bullet takeaways.
