...
FORMAT (set by Joshua 2026-08-14, replaces the one-line-per-store summary): post ONE markdown table, no commentary, no reasons, no emoji lines. Columns: Store | Category | Expected | PM Count | Variance. Expected = AM count minus sold (per category, from the Bravo bucketing). One row per category per store plus a bolded Total row per store. Variance = PM Count - Expected, signed (+/-/0). Nothing else in the message except a first line: "Jewelry Count — <Weekday> <M/D> (expected = AM count + sold adjustments)".
Prior format for reference only — do NOT use: post a short summary, one line per store —
