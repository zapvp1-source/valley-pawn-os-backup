...
**PHASE ORDER RULE (added 2026-07-22):** Phase 3 is the core deliverable. If Phase 3 cannot run (Chekkit unreachable, Chrome tools unavailable), still finish Phase 1 CSVs and leave them in place, but the run MUST NOT be considered complete — the CSVs + this runbook are everything the next session needs to finish the sends. **STALL = FAILURE (added 2026-09-07):** an incomplete run — Phase 3 not confirmed sent at all 5 stores by the end of the run, for ANY reason including a busy pipeline queue, a pending trigger, or "submitted but not confirmed" — COUNTS AS "did not complete" under the failure policy above. Send Joshua the one-line DM before ending the run. Never end a run silently as merely "incomplete/pending" — that loophole is what made the 2026-08-11 stall invisible for three weeks. (On 2026-07-21 the run pulled data and did the Brevo upload but silently skipped Phase 3; the sends were completed manually the next morning.)
