...
STEP 1.5 - FAST PATH (added 2026-08-16, bravo-morning-pull consolidation). The 6:50 AM `bravo-morning-pull` task normally has today’s items-to-price CSVs already pulled and integrity-verified. Check `logs/_morning_pull_status_<DATE>.txt` for a line `items-to-price CLEAN`; if the file exists but that line is not there yet, poll up to ~10 min (<=18s sleeps, separate calls), stopping early if the line appears as FAILED. If CLEAN is present AND all 5 `output/<DATE>_<STORE>_items-to-price.csv` exist: SKIP the health guard, STEP 2, STEP 3, and the log-based TRUNCATED half of STEP 4.5 (the certificate already verified truncation against the morning-pull log) - go straight to STEP 4 (compute) then STEP 5 (post). The MISSING check in STEP 4 still applies as written. If the certificate is absent, says FAILED, or any CSV is missing: run the normal flow below exactly as written - it is the unchanged, proven fallback.

ENSURE BRAVO HEALTHY FIRST (single-flight self-heal, added 2026-06-19): Before dropping the trigger, run the shared health guard bravo_ensure_healthy.sh (in the Bravo Data Extraction folder) via osascript, BACKGROUNDED with nohup so it cannot hang this session. Then poll logs/_health_gate_status.txt in <=18s sleeps across separate calls (cap ~8 min) until it reads PASS, and only then drop the trigger. The guard makes the data source healthy AND its lockfile guarantees only ONE recovery runs even if sibling morning tasks fire at the same time (prevents a collision). The existing reactive watcher-restart stays as a backstop.
...
STEP 4.5 — COMPLETENESS + INTEGRITY GATE (completeness half hardened 2026-06-24; integrity half added 2026-06-20). Two failure modes block a post: a store with NO CSV (MISSING / completeness failure) and a store whose grid walk bailed before capturing every row (TRUNCATED / integrity failure). Both must be zero before STEP 5.
  ...
  Otherwise (any MISSING or TRUNCATED) → RECOVERY LOOP. Do NOT post yet. Run up to 3 recovery rounds, each targeting ONLY the not-yet-clean stores:
    Round R (R = 1..3):
    1. Heal the data source: run bravo_ensure_healthy.sh backgrounded via nohup and poll logs/_health_gate_status.txt until PASS (cap ~8 min). If this is round 2 or 3 (a store failed a prior round — typically a render-timeout where the grid `counter=''` never appears), ALSO force a watcher restart before re-triggering: run `_restart_watcher.ps1` via osascript (e.g. `do shell script "/usr/bin/pwsh -File ... _restart_watcher.ps1"` or the established powershell invocation) to clear any wedged UI state, then re-run the health guard to PASS.
