...
> ⚠️ **SUPERSEDED 2026-08-09 (contradiction fix).** The 2026-06-08 policy below said "never DM, silently exit," directly contradicting the 2026-07-22 v2 policy above. Real incident: this task aborted silently 2026-08-06 and 2026-08-07, and never even fired 2026-08-08 — zero notification to Joshua for 3 days on a money-safety control. Steps 2e/4/5 below still say "do not DM" — that language is overridden by the v2 policy at the top of this file.
> 1. ~~Never DM Joshua.~~ DM once on failure per v2 policy at top.
> 2. **Never post to a shared Slack CHANNEL on failure** — still true. The one allowed notification is the single v2 DM.
> 3. **On failure, silently iterate to fix it first** — still true. Retry failed cells, restart the watcher, re-drop the trigger, wait out warming connectors — but once the time budget is exhausted, send the ONE v2 DM instead of exiting silently.
> 4. Save the markdown report file either way — still true. The file is the durable technical record; the DM is the one-line notice something needs a look.
...
After scheduling the restart, wait 90 s, then drop a fresh trigger and resume polling. Do not post to a Slack channel. If this restart cycle is the one that exhausts the time budget, send the v2 DM per Step 4/5 rather than exiting silently.
...
6. **If budget is exhausted with stores still failing:** send the ONE v2 DM (⚠️ Scheduled task "daily-funds-verification" did not complete — <date>), save the markdown report capturing whatever you have. No Slack CHANNEL post.

The retry/wait loop is the first line of failure handling. If it does not resolve within budget, the single v2 DM is the notification path — see Step 4.6.
