; ============================================================================
; reports/LoanDisposition.ahk
;
; Pulls Bravo's "Loan Disposition" report and exports it as CSV.
; Built-in Bravo report (Loan Reports section).
; Loan outcomes. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLoanDispositionEndDate(yyyymmdd)
...
global LD_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Loan Disposition",
    "config_ok",          "Ok",
...
PullLoanDisposition(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "loan-disposition",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampLoanDispositionEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LoanDisposition end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LoanDisposition range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "loan-disposition")
    ...
    try {
        ...
        ClickByName(LD_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LD_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LD_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LD_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LD_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LD_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LD_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
