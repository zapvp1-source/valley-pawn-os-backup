; ============================================================================
; reports/LayawayBalance.ahk
;
; Pulls Bravo's "Layaway Balance" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Outstanding layaway balances. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLayawayBalanceEndDate(yyyymmdd)
...
global LB_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Layaway Balance",
    "config_ok",          "Ok",
...
PullLayawayBalance(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "layaway-balance",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampLayawayBalanceEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LayawayBalance end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LayawayBalance range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "layaway-balance")
    ...
    try {
        ...
        ClickByName(LB_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LB_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LB_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LB_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LB_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LB_ELEMENTS["preview_export"], 5000)
        if !FindByName(LB_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(LB_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LB_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
