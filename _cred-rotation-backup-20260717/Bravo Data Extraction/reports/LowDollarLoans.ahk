; ============================================================================
; reports/LowDollarLoans.ahk
;
; Runs the "Claude Low Dollar Loans" saved Ad Hoc loan report for a single
...
;   -> "Claude Low Dollar Loans" -> override Start/End Date -> Enter -> grid
;
; Trigger schema (string in "date" field):
;   "YYYY-MM-DD"                       — single day
;   "YYYY-MM-DD..YYYY-MM-DD"           — explicit range
;
; Mirrors the SoldInvDetails / FpdLookback12Mo pattern. Uses SelectSavedReport
; from ChekkitInactives.ahk (shared) and SetReportDate for the date overrides.
; ============================================================================

#Requires AutoHotkey v2.0

global LOW_DOLLAR_LOANS_ELEMENTS := Map(
    "sidebar_loans_buys",      "Loans/Buys",
    "panel_custom_reports",    "Custom Reports",
    "saved_report_combo",      "Choose Saved Report",
    "saved_report_value",      "Claude Low Dollar Loans",
    "dialog_ok",               "Ok",
    "panel_cancel",            "Cancel"
)

PullLowDollarLoans(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "low-dollar-loans",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    LogMessage("[" . store . "] LowDollarLoans startDate=" . startDate . " endDate=" . endDate)

    outputFileName := startDate . "_to_" . endDate . "_" . store . "_low-dollar-loans.csv"
    ...
    ; Pre-dismiss stuck dialogs (mirrors SoldInvDetails defensive cleanup;
    ...
    try {
        ...
        ClickByName(LOW_DOLLAR_LOANS_ELEMENTS["sidebar_loans_buys"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: click Custom Reports")
        ClickByName(LOW_DOLLAR_LOANS_ELEMENTS["panel_custom_reports"], 5000)
        Sleep(1500)

        LogMessage("  step 3: select 'Claude Low Dollar Loans' saved report")
        SelectSavedReport(LOW_DOLLAR_LOANS_ELEMENTS["saved_report_combo"], LOW_DOLLAR_LOANS_ELEMENTS["saved_report_value"])
        Sleep(1000)
        ...
        if (count = 0 && !gridReady) {
            ...
            if (!gridReady) {
                LogVisibleNames()
                throw Error("Low-dollar loans grid did not render within 120s after Ok click")
            }
            Sleep(1500)
        ...
        try ClickByName(LOW_DOLLAR_LOANS_ELEMENTS["panel_cancel"], 3000)
        Sleep(800)
        try ClickByName(LOW_DOLLAR_LOANS_ELEMENTS["panel_cancel"], 3000)
        Sleep(800)
    } catch as e {
