; ============================================================================
; reports/SoldInventory.ahk
;
; Pulls Bravo's "Sold Inventory" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Period sales detail. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampSoldInventoryEndDate(yyyymmdd)
...
PullSoldInventory(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "sold-inventory",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampSoldInventoryEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] SoldInventory end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] SoldInventory range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "sold-inventory")
    ...
    try {
        ...
        ClickByName(SI_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(SI_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(SI_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(SI_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(SI_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(SI_ELEMENTS["preview_export"], 5000)
        if !FindByName(SI_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(SI_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(SI_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
