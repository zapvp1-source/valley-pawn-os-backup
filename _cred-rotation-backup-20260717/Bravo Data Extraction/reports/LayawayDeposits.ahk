; ============================================================================
; reports/LayawayDeposits.ahk
;
; Pulls Bravo's "Layaway Deposits" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Layaway payments received. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLayawayDepositsEndDate(yyyymmdd)
...
PullLayawayDeposits(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "layaway-deposits",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampLayawayDepositsEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LayawayDeposits end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LayawayDeposits range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "layaway-deposits")
