; ============================================================================
; reports/EndOfDay.ahk
;
; Pulls Bravo's "End of Day" report and exports it as CSV.
; Built-in Bravo report (Closing Reports section).
; Likely SINGLE date dialog. SetReportDate(2) will WARN harmlessly.
;
...
global EOD_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "End of Day",
    "config_ok",          "Ok",
...
PullEndOfDay(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "end-of-day",
        "store",       store,
    ...
    endDateIso := ClampEndOfDayEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] EndOfDay end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] EndOfDay range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "end-of-day")
    ...
    try {
        ...
        ClickByName(EOD_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(EOD_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(EOD_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(EOD_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(EOD_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(EOD_ELEMENTS["preview_export"], 5000)
        if !FindByName(EOD_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(EOD_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(EOD_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
