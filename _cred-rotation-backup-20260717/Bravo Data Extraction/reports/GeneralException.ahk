; ============================================================================
; reports/GeneralException.ahk
;
; Pulls Bravo's "General Exception" report and exports it as CSV.
...
; ----------------------------------------------------------------------------
; ClampGeneralExceptionEndDate(yyyymmdd)
...
global GE_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "General Exception",
    "config_ok",          "Ok",
...
PullGeneralException(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampGeneralExceptionEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] GeneralException end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] GeneralException range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "general-exception")
    ...
    try {
        ...
        ClickByName(GE_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(GE_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(GE_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(GE_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(GE_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(GE_ELEMENTS["preview_export"], 5000)
        if !FindByName(GE_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(GE_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(GE_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
