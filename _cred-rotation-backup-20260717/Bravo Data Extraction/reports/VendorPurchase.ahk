; ============================================================================
; reports/VendorPurchase.ahk
;
; Pulls Bravo's "Vendor Purchase" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Wholesale buys. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampVendorPurchaseEndDate(yyyymmdd)
...
global VP_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Vendor Purchase",
    "config_ok",          "Ok",
...
PullVendorPurchase(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "vendor-purchase",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampVendorPurchaseEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] VendorPurchase end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] VendorPurchase range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "vendor-purchase")
    ...
    try {
        ...
        ClickByName(VP_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(VP_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(VP_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(VP_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(VP_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(VP_ELEMENTS["preview_export"], 5000)
        if !FindByName(VP_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(VP_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(VP_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
