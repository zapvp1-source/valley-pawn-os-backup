; ============================================================================
; reports/CostAdjustment.ahk
;
; Pulls Bravo's "Cost Adjustment" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Closing-style. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampCostAdjustmentEndDate(yyyymmdd)
...
global CA_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Cost Adjustment",
    "config_ok",          "Ok",
...
PullCostAdjustment(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampCostAdjustmentEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] CostAdjustment end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] CostAdjustment range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "cost-adjustment")
    ...
    try {
        ...
        ClickByName(CA_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(CA_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(CA_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(CA_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(CA_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(CA_ELEMENTS["preview_export"], 5000)
        if !FindByName(CA_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(CA_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(CA_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
