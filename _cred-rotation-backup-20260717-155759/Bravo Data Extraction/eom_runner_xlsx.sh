...
LOG="$BASE/logs/_eom_runner_xlsx.log"
...
for S in $STORES; do
  ...
  for a in $(seq 1 $MAX_ATTEMPTS); do
    ...
    killbravo; sleep 6
    RF="$RES/eomxlsx-$S.result.json"; rm -f "$RF" 2>/dev/null
    CSV="$OUT/${ENDDATE}_${S}_end-of-month.xlsx"; rm -f "$CSV" 2>/dev/null
    printf '{"id":"eomxlsx-%s","requested_at":"%s","reports":[{"name":"end-of-month","stores":["%s"],"date":"%s"}]}' \
        "$S" "$(date '+%Y-%m-%dT%H:%M:%S-04:00')" "$S" "$RANGE" > "$TRIG/eomxlsx-$S.json"
