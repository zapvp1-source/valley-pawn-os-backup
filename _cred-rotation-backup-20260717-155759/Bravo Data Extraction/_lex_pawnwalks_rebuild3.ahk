...
; Result: logs\_lexpw_rebuild3_result.txt = SAVED / NOSAVE <reason> / FAIL <reason>
...
global RES := CONFIG["paths.logs"] . "\_lexpw_rebuild3_result.txt"
WriteRes(s) {
    global RES
    try FileDelete(RES)
...
InitLog(CONFIG["paths.logs"], "lexpw-rebuild3-" . FormatTime(, "yyyyMMdd-HHmmss"))
LogMessage("=== LEX REBUILD v3 start ===")
WriteRes("RUNNING")
...
try {
    if TextExistsInBox("Disposition Date", 500, 600, 700, 1700) {
        LogMessage("step3: Disposition Date row already present - not re-adding")
    } else {
...
; --- 5. criteria: Ticket Kind = BUY (same dialog session) -------------------
kindOk := false
try {
    preY := TextYInBox("Ticket Kind", 500, 600, 700, 1700)
    if (preY >= 0) {
        v0 := ""
        try v0 := FindComboNearRow(preY).Value
        if InStr(v0, "BUY") {
            kindOk := true
            LogMessage("step5: Ticket Kind=BUY already present - skipping")
        }
    }
    if !kindOk {
    if (preY < 0) {
    ...
    LogMessage("step5: Ticket Kind row present")
    ...
    LogMessage("step5 (Ticket Kind) FAILED: " . e.Message)
}

; --- 6. STRICT GATE + SINGLE SAVE ---------------------------------------------------
LogMessage("--- pre-save: layoutOk=" . layoutOk . " dispOk=" . dispOk . " datesOk=" . datesOk . " kindOk=" . kindOk . " ---")
if !(layoutOk && dispOk && datesOk = 2 && kindOk) {
    WriteRes("NOSAVE layoutOk=" . layoutOk . " dispOk=" . dispOk . " datesOk=" . datesOk . " kindOk=" . kindOk)
    LogMessage("strict gate failed - cancelling, nothing saved")
    CleanExit(1)
}
saveBtn := FindButtonInBox("Save", 2100, 450, 2450, 700)
if !saveBtn {
    WriteRes("NOSAVE Save button not found")
    CleanExit(1)
}
try {
    saveBtn.Click("left")
    Sleep(1800)
    LogMessage("step6: clicked Save")
    try DismissPopups()
    Sleep(1000)
} catch as e {
...
; --- 7. VERIFY: reload the report fresh and confirm the saved definition ------------
verified := false
try {
    ; make sure the dialog is open (Save may or may not close it)
    if !FindComboByAid("BoxSelectCriteria") {
        DismissPopups()
        ClickByName("Custom Reports", 5000)
        Sleep(1500)
    }
    c3 := FindBottomSavedReportCombo()
    if c3 {
        c3.Click("left")
        Sleep(900)
    }
    ...
    Sleep(1200)
    vCol := ""
    try vCol := FindComboByAid("BoxColumns").Value
    vDisp := TextExistsInBox("Disposition Date", 500, 600, 700, 1700)
    vRange := TextExistsInBox("range", 700, 600, 1300, 1700)
    vKindY := TextYInBox("Ticket Kind", 500, 600, 700, 1700)
    vKindVal := ""
    if (vKindY >= 0)
        try vKindVal := FindComboNearRow(vKindY).Value
    LogMessage("verify: BoxColumns=" . vCol)
    LogMessage("verify: disp=" . vDisp . " range=" . vRange . " kindY=" . vKindY . " kindVal=" . vKindVal)
    DumpTextsInBox(400, 600, 2400, 1300)
    DumpEditsAll()
    if (InStr(vCol, "490f2277") && vDisp && vRange && vKindY >= 0 && InStr(vKindVal, "BUY"))
        verified := true
} catch as e {
    LogMessage("verify FAILED: " . e.Message)
}
WriteRes(verified ? "SAVED+VERIFIED" : "SAVED but verify incomplete - check log")
LogMessage("=== REBUILD v3 done (verified=" . verified . ") ===")
CleanExit(verified ? 0 : 1)
