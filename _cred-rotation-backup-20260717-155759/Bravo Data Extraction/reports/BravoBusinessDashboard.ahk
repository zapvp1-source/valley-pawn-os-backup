; ============================================================================
; reports/BravoBusinessDashboard.ahk
;
; Pulls Bravo's "BRAVO Business Dashboard" report and exports it as CSV.
; Built-in Bravo report (Closing Reports section).
; SINGLE date dialog. SetReportDate(2) will WARN harmlessly.
;
...
; ----------------------------------------------------------------------------
; ClampBravoBusinessDashboardEndDate(yyyymmdd)
...
global BBD_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "BRAVO Business Dashboard",
    "config_ok",          "Ok",
...
PullBravoBusinessDashboard(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampBravoBusinessDashboardEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] BravoBusinessDashboard end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] BravoBusinessDashboard range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "bravo-business-dashboard")
    ...
    try {
        ...
        ClickByName(BBD_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(BBD_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(BBD_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(BBD_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(BBD_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(BBD_ELEMENTS["preview_export"], 5000)
        if !FindByName(BBD_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(BBD_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(BBD_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
