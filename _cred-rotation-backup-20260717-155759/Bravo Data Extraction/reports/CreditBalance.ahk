; ============================================================================
; reports/CreditBalance.ahk
;
; Pulls Bravo's "Credit Balance" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Customer credit liability. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampCreditBalanceEndDate(yyyymmdd)
...
global CB_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Credit Balance",
    "config_ok",          "Ok",
...
PullCreditBalance(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampCreditBalanceEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] CreditBalance end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] CreditBalance range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "credit-balance")
    ...
    try {
        ...
        ClickByName(CB_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(CB_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(CB_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(CB_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(CB_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(CB_ELEMENTS["preview_export"], 5000)
        if !FindByName(CB_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(CB_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(CB_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
