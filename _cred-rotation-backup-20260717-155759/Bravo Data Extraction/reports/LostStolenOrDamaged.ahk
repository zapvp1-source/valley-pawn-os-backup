; ============================================================================
; reports/LostStolenOrDamaged.ahk
;
; Pulls Bravo's "Lost Stolen or Damaged" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Shrink reporting. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLostStolenOrDamagedEndDate(yyyymmdd)
...
global LSOD_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Lost Stolen or Damaged",
    "config_ok",          "Ok",
...
PullLostStolenOrDamaged(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampLostStolenOrDamagedEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LostStolenOrDamaged end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LostStolenOrDamaged range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "lost-stolen-or-damaged")
    ...
    try {
        ...
        ClickByName(LSOD_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LSOD_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LSOD_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LSOD_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LSOD_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LSOD_ELEMENTS["preview_export"], 5000)
        if !FindByName(LSOD_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(LSOD_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LSOD_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
