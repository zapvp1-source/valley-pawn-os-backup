; ============================================================================
; reports/SalesAccounting.ahk
;
; Pulls Bravo's "Sales Accounting" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Accounting view of sales. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampSalesAccountingEndDate(yyyymmdd)
...
global SA_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Sales Accounting",
    "config_ok",          "Ok",
...
PullSalesAccounting(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "sales-accounting",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampSalesAccountingEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] SalesAccounting end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] SalesAccounting range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "sales-accounting")
    ...
    try {
        ...
        ClickByName(SA_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(SA_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(SA_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(SA_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(SA_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(SA_ELEMENTS["preview_export"], 5000)
        if !FindByName(SA_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(SA_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(SA_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
