; ============================================================================
; reports/RetailReportsDashboard.ahk
;
; Pulls Bravo's "Retail Reports Dashboard" report and exports it as CSV.
; Built-in Bravo report (Retail Reports section).
; Retail dashboard. May be SSRS — verify in overnight run.
;
...
; ----------------------------------------------------------------------------
; ClampRetailReportsDashboardEndDate(yyyymmdd)
...
global RRD_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Retail Reports Dashboard",
    "config_ok",          "Ok",
...
PullRetailReportsDashboard(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "retail-reports-dashboard",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampRetailReportsDashboardEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] RetailReportsDashboard end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] RetailReportsDashboard range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "retail-reports-dashboard")
    ...
    try {
        ...
        ClickByName(RRD_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(RRD_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(RRD_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(RRD_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(RRD_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(RRD_ELEMENTS["preview_export"], 5000)
        if !FindByName(RRD_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(RRD_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(RRD_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
