; ============================================================================
; reports/LargeCashTransactions.ahk
;
; Pulls Bravo's "Large Cash Transactions" report and exports it as CSV.
...
; ----------------------------------------------------------------------------
; ClampLargeCashTransactionsEndDate(yyyymmdd)
...
global LCT_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Large Cash Transactions",
    "config_ok",          "Ok",
...
PullLargeCashTransactions(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampLargeCashTransactionsEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LargeCashTransactions end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LargeCashTransactions range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "large-cash-transactions")
    ...
    try {
        ...
        ClickByName(LCT_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LCT_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LCT_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LCT_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LCT_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LCT_ELEMENTS["preview_export"], 5000)
        if !FindByName(LCT_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(LCT_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LCT_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
