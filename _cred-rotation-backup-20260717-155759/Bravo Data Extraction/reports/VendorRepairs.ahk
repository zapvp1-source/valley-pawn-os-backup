; ============================================================================
; reports/VendorRepairs.ahk
;
; Pulls Bravo's "Vendor Repairs" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Items out for repair. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampVendorRepairsEndDate(yyyymmdd)
...
PullVendorRepairs(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "vendor-repairs",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampVendorRepairsEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] VendorRepairs end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] VendorRepairs range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "vendor-repairs")
    ...
    try {
        ...
        ClickByName(VR_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(VR_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(VR_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(VR_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(VR_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(VR_ELEMENTS["preview_export"], 5000)
        if !FindByName(VR_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(VR_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(VR_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
