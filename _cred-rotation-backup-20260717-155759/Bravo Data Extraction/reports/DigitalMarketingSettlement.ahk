; ============================================================================
; reports/DigitalMarketingSettlement.ahk
;
; Pulls Bravo's "Digital Marketing Settlement" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Online marketing reconciliation. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampDigitalMarketingSettlementEndDate(yyyymmdd)
...
global DMS_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Digital Marketing Settlement",
    "config_ok",          "Ok",
...
PullDigitalMarketingSettlement(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "digital-marketing-settlement",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampDigitalMarketingSettlementEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] DigitalMarketingSettlement end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] DigitalMarketingSettlement range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "digital-marketing-settlement")
    ...
    try {
        ...
        ClickByName(DMS_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(DMS_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(DMS_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(DMS_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(DMS_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(DMS_ELEMENTS["preview_export"], 5000)
        if !FindByName(DMS_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(DMS_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(DMS_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
