; ============================================================================
; reports/DisbursementJournal.ahk
;
; Pulls Bravo's "Disbursement Journal" report and exports it as CSV.
...
; ----------------------------------------------------------------------------
; ClampDisbursementJournalEndDate(yyyymmdd)
...
global DJ_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Disbursement Journal",
    "config_ok",          "Ok",
...
PullDisbursementJournal(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampDisbursementJournalEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] DisbursementJournal end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] DisbursementJournal range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "disbursement-journal")
    ...
    try {
        ...
        ClickByName(DJ_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(DJ_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(DJ_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(DJ_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(DJ_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(DJ_ELEMENTS["preview_export"], 5000)
        if !FindByName(DJ_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(DJ_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(DJ_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
