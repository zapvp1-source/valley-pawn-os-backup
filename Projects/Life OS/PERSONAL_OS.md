...
**`Health Optimization`** project folder (69 files as of 2026-08-07) is an active, deep personal
health investigation — lab panels (Function Health, Genova GI Effects), a CGM (continuous glucose
monitor) data stream, Oura ring data (also auto-imported daily via the `oura-daily-import`
scheduled task), an ER visit record (2026-07-28), and multiple synthesized findings documents.

**Start here, not by re-reading raw data:**
- `Master_Findings_Index.md` — the index into everything else in the folder
- `Health_Folder_Findings.md`, `Data_Coverage_Ledger.md` — what data exists and what it shows
- `Case_Summary_SOURCED.md` / `Case_Summary_for_Dr_Wilson.docx` — the consolidated case summary
  prepared for a physician
- `BOARD_CONSENSUS_June2026.md`, `Board_A_Conventional_SteelmanStrawman.md`,
  `Board_B_Fringe_Heterodox_Panel.md`, `Boards_Reconvened_June2026.md` — Joshua has been running
  an expert-review-board-style process on his own health questions, weighing conventional vs.
  heterodox perspectives. Consistent with how he wants technical/business decisions handled
  (reasoned recommendation, not raw questions) — same pattern applied to his own health research.

**Handling note:** this is sensitive personal medical material. When working in this folder,
stay factual and sourced (same standard as the rest of Joshua's operation — verify against
actual data, don't diagnose from metadata). Do not add speculative medical interpretation beyond
what's already in the findings documents; flag genuine medical decisions as Joshua's to make with
his doctors, not Claude's to resolve.
...
- **`Taxes 2026`** project folder is cross-domain (see `Life OS/LIFE_MAP.md` for why) — it holds
  ...
  - Cypress Crossing and Bald Rock capital-improvement evidence logs (detailed in `Real Estate
    OS` — Bald Rock is FCF Inc money, Cypress Crossing is personal money; both get substantiated
    the same way)
