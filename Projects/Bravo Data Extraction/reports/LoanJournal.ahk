; ============================================================================
; reports/LoanJournal.ahk
;
; Pulls Bravo's "Loan Journal" report and exports it as CSV.
; Built-in Bravo report (Loan Reports section).
; All loan transactions. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLoanJournalEndDate(yyyymmdd)
...
PullLoanJournal(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "loan-journal",
        "store",       store,
    ...
    endDateIso := ClampLoanJournalEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LoanJournal end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LoanJournal range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "loan-journal")
