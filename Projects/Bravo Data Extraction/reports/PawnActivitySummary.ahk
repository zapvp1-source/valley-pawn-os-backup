; ============================================================================
; reports/PawnActivitySummary.ahk
;
; Pulls Bravo's "Pawn Activity Summary" report and exports it as CSV.
; Built-in Bravo report (Loan Reports section).
; Period summary. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampPawnActivitySummaryEndDate(yyyymmdd)
...
global PAS_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Pawn Activity Summary",
    "config_ok",          "Ok",
...
PullPawnActivitySummary(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "pawn-activity-summary",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampPawnActivitySummaryEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] PawnActivitySummary end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] PawnActivitySummary range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "pawn-activity-summary")
    ...
    try {
        ...
        ClickByName(PAS_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(PAS_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(PAS_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(PAS_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(PAS_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(PAS_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(PAS_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
