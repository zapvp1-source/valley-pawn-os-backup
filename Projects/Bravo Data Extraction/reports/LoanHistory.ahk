; ============================================================================
; reports/LoanHistory.ahk
;
; Pulls Bravo's "Loan History" report and exports it as CSV.
; Built-in Bravo report (Loan Reports section).
; Loan/customer history. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLoanHistoryEndDate(yyyymmdd)
...
global LH_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Loan History",
    "config_ok",          "Ok",
...
PullLoanHistory(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "loan-history",
        "store",       store,
    ...
    endDateIso := ClampLoanHistoryEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LoanHistory end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LoanHistory range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "loan-history")
    ...
    try {
        ...
        ClickByName(LH_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LH_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LH_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LH_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LH_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LH_ELEMENTS["preview_export"], 5000)
        if !FindByName(LH_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(LH_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LH_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
