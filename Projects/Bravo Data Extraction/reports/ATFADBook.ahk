; ============================================================================
; reports/ATFADBook.ahk
;
; Pulls Bravo's "ATF A & D Book" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Firearms compliance. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampATFADBookEndDate(yyyymmdd)
...
global ATFADB_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "ATF A & D Book",
    "config_ok",          "Ok",
...
PullATFADBook(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "atf-ad-book",
        "store",       store,
    ...
    endDateIso := ClampATFADBookEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] ATFADBook end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] ATFADBook range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "atf-ad-book")
    ...
    try {
        ...
        ClickByName(ATFADB_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(ATFADB_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(ATFADB_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(ATFADB_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(ATFADB_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(ATFADB_ELEMENTS["preview_export"], 5000)
        if !FindByName(ATFADB_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(ATFADB_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(ATFADB_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
