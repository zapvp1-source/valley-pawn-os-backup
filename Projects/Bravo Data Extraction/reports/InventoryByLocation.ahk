; ============================================================================
; reports/InventoryByLocation.ahk
;
; Pulls Bravo's "Inventory by Location" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Where items physically sit. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampInventoryByLocationEndDate(yyyymmdd)
...
global IBL_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Inventory by Location",
    "config_ok",          "Ok",
...
PullInventoryByLocation(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampInventoryByLocationEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] InventoryByLocation end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] InventoryByLocation range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "inventory-by-location")
    ...
    try {
        ...
        ClickByName(IBL_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(IBL_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(IBL_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(IBL_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(IBL_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(IBL_ELEMENTS["preview_export"], 5000)
        if !FindByName(IBL_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(IBL_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(IBL_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
