; ============================================================================
; reports/InterStoreCashTransfer.ahk
;
; Pulls Bravo's "Inter-Store Cash Transfer" report and exports it as CSV.
...
; ----------------------------------------------------------------------------
; ClampInterStoreCashTransferEndDate(yyyymmdd)
...
global ISCT_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Inter-Store Cash Transfer",
    "config_ok",          "Ok",
...
PullInterStoreCashTransfer(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "inter-store-cash-transfer",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampInterStoreCashTransferEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] InterStoreCashTransfer end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] InterStoreCashTransfer range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "inter-store-cash-transfer")
    ...
    try {
        ...
        ClickByName(ISCT_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(ISCT_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(ISCT_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(ISCT_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(ISCT_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(ISCT_ELEMENTS["preview_export"], 5000)
        if !FindByName(ISCT_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(ISCT_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(ISCT_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
