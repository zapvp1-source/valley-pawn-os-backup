; ============================================================================
; reports/DepositsAndPaidOuts.ahk
;
; Pulls Bravo's "Deposits and Paid Outs Spreadsheet" report and exports it as CSV.
...
; ----------------------------------------------------------------------------
; ClampDepositsAndPaidOutsEndDate(yyyymmdd)
...
global DAPO_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Deposits and Paid Outs Spreadsheet",
    "config_ok",          "Ok",
...
PullDepositsAndPaidOuts(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "deposits-paid-outs",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampDepositsAndPaidOutsEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] DepositsAndPaidOuts end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] DepositsAndPaidOuts range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "deposits-paid-outs")
    ...
    try {
        ...
        ClickByName(DAPO_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(DAPO_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(DAPO_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(DAPO_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(DAPO_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(DAPO_ELEMENTS["preview_export"], 5000)
        if !FindByName(DAPO_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(DAPO_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(DAPO_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
