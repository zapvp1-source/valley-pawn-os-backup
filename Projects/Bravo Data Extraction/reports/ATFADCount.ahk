; ============================================================================
; reports/ATFADCount.ahk
;
; Pulls Bravo's "ATF A & D Count" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Firearms compliance count. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampATFADCountEndDate(yyyymmdd)
...
global ATFADC_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "ATF A & D Count",
    "config_ok",          "Ok",
...
PullATFADCount(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "atf-ad-count",
        "store",       store,
    ...
    endDateIso := ClampATFADCountEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] ATFADCount end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] ATFADCount range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "atf-ad-count")
    ...
    try {
        ...
        ClickByName(ATFADC_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(ATFADC_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(ATFADC_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(ATFADC_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(ATFADC_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(ATFADC_ELEMENTS["preview_export"], 5000)
        if !FindByName(ATFADC_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(ATFADC_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(ATFADC_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
