; ============================================================================
; reports/DropShipSettlement.ahk
;
; Pulls Bravo's "Drop Ship Settlement" report and exports it as CSV.
; Built-in Bravo report (Retail Reports section).
; Drop-ship reconciliation. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampDropShipSettlementEndDate(yyyymmdd)
...
global DSS_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Drop Ship Settlement",
    "config_ok",          "Ok",
...
PullDropShipSettlement(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "drop-ship-settlement",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampDropShipSettlementEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] DropShipSettlement end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] DropShipSettlement range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "drop-ship-settlement")
    ...
    try {
        ...
        ClickByName(DSS_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(DSS_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(DSS_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(DSS_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(DSS_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(DSS_ELEMENTS["preview_export"], 5000)
        if !FindByName(DSS_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(DSS_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(DSS_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
