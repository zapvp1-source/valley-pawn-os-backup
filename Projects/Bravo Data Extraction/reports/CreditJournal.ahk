; ============================================================================
; reports/CreditJournal.ahk
;
; Pulls Bravo's "Credit Journal" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Credit detail. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampCreditJournalEndDate(yyyymmdd)
...
global CJ_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Credit Journal",
    "config_ok",          "Ok",
...
PullCreditJournal(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "credit-journal",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampCreditJournalEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] CreditJournal end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] CreditJournal range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "credit-journal")
    ...
    try {
        ...
        ClickByName(CJ_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(CJ_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(CJ_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(CJ_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(CJ_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(CJ_ELEMENTS["preview_export"], 5000)
        if !FindByName(CJ_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(CJ_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(CJ_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
