; ============================================================================
; reports/InventoryBase.ahk
;
; Pulls Bravo's "Inventory Base" report and exports it as CSV.
; Built-in Bravo report (Inventory Reports section).
; Closing-style snapshot. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampInventoryBaseEndDate(yyyymmdd)
...
global IB_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Inventory Base",
    "config_ok",          "Ok",
...
PullInventoryBase(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
    ...
    endDateIso := ClampInventoryBaseEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] InventoryBase end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] InventoryBase range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "inventory-base")
    ...
    try {
        ...
        ClickByName(IB_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(IB_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(IB_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(IB_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(IB_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(IB_ELEMENTS["preview_export"], 5000)
        if !FindByName(IB_ELEMENTS["export_ok"], 8000)
        ...
        ClickByName(IB_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(IB_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
