; ============================================================================
; reports/LayawayJournal.ahk
;
; Pulls Bravo's "Layaway Journal" report and exports it as CSV.
; Built-in Bravo report (Sales Reports section).
; Layaway transactions. Handles both single and range dialogs.
;
...
; ----------------------------------------------------------------------------
; ClampLayawayJournalEndDate(yyyymmdd)
...
global LJ_ELEMENTS := Map(
    "sidebar_reports",    "Reports",
    "report_tile",        "Layaway Journal",
    "config_ok",          "Ok",
...
PullLayawayJournal(store, dateOrRange, outputDir) {
    started := A_TickCount
    result := Map(
        "report",      "layaway-journal",
        "store",       store,
        "date",        dateOrRange,
        "status",      "error",
        "output_path", "",
    ...
    endDateIso := ClampLayawayJournalEndDate(endDateIso)
    if (endDateIso != origEndDateIso)
        LogMessage("[" . store . "] LayawayJournal end date clamped: " . origEndDateIso . " -> " . endDateIso . " (Bravo rejects today/future)")
    LogMessage("[" . store . "] LayawayJournal range=" . startDateIso . ".." . endDateIso)

    outputPath := outputDir . "\" . OutputFilename(endDateIso, store, "layaway-journal")
    ...
    try {
        ...
        ClickByName(LJ_ELEMENTS["sidebar_reports"], 8000)
        Sleep(1500)
        DismissPopups()

        LogMessage("  step 2: double-click End of Month tile (= Preview)")
        DoubleClickByName(LJ_ELEMENTS["report_tile"], 8000)
        Sleep(2500)
        DismissPopups()
        ...
        if !FindByName(LJ_ELEMENTS["preview_export"], 0) {
            LogMessage("    [fallback] Preview not yet visible, trying ClickByName(Ok)")
            try {
                ClickByName(LJ_ELEMENTS["config_ok"], 5000)
        ...
        if !FindByName(LJ_ELEMENTS["preview_export"], 60000)
        ...
        ClickByName(LJ_ELEMENTS["export_ok"], 5000)
        ...
        Loop 3 {
            if ExistsByName("Reports")
                break
            try ClickByName(LJ_ELEMENTS["panel_done"], 3000)
            Sleep(1200)
            DismissPopups()
        }
