...
- 2026-08-03: CRITICAL - post-to-accounting-gl (Consolidated General Ledger) hangs on preview render for EVERY store attempted, blocking the sales-tax-monthly-update task entirely. FIX APPLIED same day: Continuous-Scrolling toggle-off block ported into reports/PostToAccountingGL.ahk (backup: PostToAccountingGL.ahk.bak-pre-cs-fix-2026-08-03). NOT YET LIVE-VERIFIED - next session to run post-to-accounting-gl or eom-bravo-gl-export MUST confirm no hang and stamp this SOLVED. See full writeup below.
