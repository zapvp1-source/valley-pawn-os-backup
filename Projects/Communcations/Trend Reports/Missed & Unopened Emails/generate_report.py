#!/usr/bin/env python3
"""
Valley Pawn — Missed & Unopened Emails trend report generator.

Modeled directly on the sibling "Missed Calls & Voicemails" generator in this same
Trend Reports folder — same schema shape, same visual language, so the two reports
read as a matched pair.

Reads daily_log.csv (one row per store per day: date,store,candidates,resolved,
unresolved,opened_pct) sitting alongside this script, and writes report.html in the
same folder — a single self-contained HTML file (Chart.js via CDN) showing:

  - Summary cards (all-time totals, overall same-day-open %, days tracked)
  - Daily emails received by store (stacked bar)
  - Emails by month, by store (grouped bar)
  - Running year-to-date cumulative total (line)
  - Same-day-opened % trend, daily by store + overall (line)
  - Full data table

Schema semantics (email equivalent of the calls report):
  candidates  = inbound emails that landed in a store INBOX that day
  resolved    = of those, how many were opened the same day (proxy for "handled",
                same role as a same-day callback in the calls report)
  unresolved  = candidates - resolved — still sitting unopened as of the EOD sweep
...
def load_rows():
    ...
    with open(CSV_PATH, newline="") as f:
        for r in csv.DictReader(f):
            ...
            r["opened_pct"] = float(r["opened_pct"])
            rows.append(r)
...
def build(rows):
    ...
    opened_daily_overall = []
    for d in dates:
        cand = sum(daily[d].values())
        res = sum(daily_resolved[d].values())
        pct = round(res / cand * 100, 1) if cand else None
        opened_daily_overall.append({"date": d, "pct": pct})

    opened_daily_by_store = defaultdict(list)
    ...
    for store in stores:
        for d in dates:
            ...
            pct = round(res / cand * 100, 1) if cand else None
            opened_daily_by_store[store].append({"date": d, "pct": pct})
    ...
    return {
        ...
        "opened_daily_overall": opened_daily_overall,
        "opened_daily_by_store": opened_daily_by_store,
...
def render(data):
    ...
    opened_datasets = [{
        ...
        "backgroundColor": "transparent",
        "borderWidth": 2.5,
        "data": [p["pct"] for p in data["opened_daily_overall"]],
        "spanGaps": True,
    }]
    for store in stores:
        opened_datasets.append({
            "label": store,
            "borderColor": color_for(store),
            "backgroundColor": "transparent",
            "borderDash": [4, 3],
            "data": [p["pct"] for p in data["opened_daily_by_store"][store]],
            "spanGaps": True,
        })
    ...
    cards = "".join(f"""
      <div class="card">
        ...
        <div class="card-sub">{s['resolved']}/{s['candidates']} opened same-day &middot; {s['unresolved']} left unopened</div>
...
<head>
<meta charset="UTF-8">
<title>Valley Pawn — Missed &amp; Unopened Emails Trend Report</title>
...
<body>
  <h1>Missed &amp; Unopened Emails — Trend Report</h1>
  <div class="meta">Full Circle Finance Inc DBA Valley Pawn &middot; generated {generated} &middot; source: store INBOX sweep (Apple Mail, 5 store accounts) via the daily EOD review &middot; companion to the Missed Calls &amp; Voicemails report</div>

  <div class="cards">
    <div class="card total">
      <div class="card-label">All stores, all time</div>
      <div class="card-value">{data['overall_pct']}%</div>
      <div class="card-sub">{data['total_resolved']}/{data['total_candidates']} opened same-day &middot; {data['total_unresolved']} left unopened</div>
    </div>{cards}
  </div>

  <div class="panel">
    <h2>Daily emails received by store</h2>
    <canvas id="dailyChart"></canvas>
  </div>

  <div class="panel">
    <h2>Emails by month, by store</h2>
  ...
  <div class="panel">
    <h2>Daily detail</h2>
    <table>
      <thead><tr><th>Date</th><th>Store</th><th class="num">Received</th><th class="num">Opened same-day</th><th class="num">Left unopened</th><th class="num">Opened %</th></tr></thead>
...
new Chart(document.getElementById('dailyChart'), {{
  ...
  options: {{ responsive:true, scales: {{ x: {{ stacked:true }}, y: {{ stacked:true, beginAtZero:true, title:{{display:true,text:'emails received'}} }} }} }}
}});

new Chart(document.getElementById('monthlyChart'), {{
  type: 'bar',
  data: {{ labels: months, datasets: {json.dumps(monthly_datasets)} }},
  options: {{ responsive:true, scales: {{ x: {{ stacked:false }}, y: {{ beginAtZero:true, title:{{display:true,text:'emails received'}} }} }} }}
}});
...
new Chart(document.getElementById('ytdChart'), {{
  ...
  options: {{ responsive:true, parsing:false,
    scales: {{ x: {{ type:'category', labels: dates }}, y: {{ beginAtZero:true, title:{{display:true,text:'cumulative emails (YTD)'}} }} }} }}
}});

new Chart(document.getElementById('openedChart'), {{
  type: 'line',
  data: {{ labels: dates, datasets: {json.dumps(opened_datasets)} }},
  options: {{ responsive:true, scales: {{ y: {{ min:0, max:100, title:{{display:true,text:'opened same-day %'}} }} }} }}
}});
</script>
</body>
</html>
