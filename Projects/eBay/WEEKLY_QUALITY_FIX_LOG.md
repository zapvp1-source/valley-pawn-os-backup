...
- Reverted 2 Waynesboro titles that had gone through the 8/10 run's mechanical title-stripper + caps-fixer pipeline, back to their true original (pre-strip) titles via `ReviseFixedPriceItem`, confirmed live via `GetMyeBaySelling`:
  ...
  - **Known caps-fixer issue surfaced by this**: `ebay_caps_fixer.py`'s `KEEP` set doesn't include `SSD`, and its `tc()` word-caser doesn't special-case abbreviations with periods like `W.R.` — both get flattened to `Ssd` / `W.r.` on any ALL-CAPS title it touches. Not patched yet (scope was just the 2-item revert); flag for the next `ebay-weekly-quality-fix` build session to add `SSD` to `KEEP` and add a rule to preserve all-caps period-abbreviated tokens (e.g. regex `^[A-Z]\.([A-Z]\.)+$`) before running caps-fixer on more listings.
