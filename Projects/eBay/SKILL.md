...
- **API access (the real automation path):** eBay **Trading API** with per-store user tokens, loaded from `~/.vp_secrets/ebay_store_tokens.py` (`STORES` list + `APP_ID/DEV_ID/CERT_ID`; `~/ebay_weekly_rankings.py` re-exports the same names for legacy consumers — never `exec()` it). Pulls all 5 stores with no login. **Limitation:** Trading tokens cannot carry REST scopes — seller standards, Promoted Listings, finances and traffic data need the per-store OAuth consent described below.
- **DEPARTMENT MAP (read first for any "is X running / what does Y do" question):** `Projects/eBay/EBAY_DEPARTMENT_PLAN_2026-09-05.md` §2 is the complete, verified inventory of every launchd agent, Cowork task, script, channel and state file — and §5–6 is the target architecture (one native engine: snapshot → rules → apply → publish). Check it before adding any new eBay task or script; the plan's whole point is to stop adding parallel writers.
- **Secrets:** everything reads from `~/.vp_secrets/` (`ebay_store_tokens.py` = 5 Trading tokens + app keys; `ebay_oauth_tokens.py` = REST OAuth; `slack_webhook_ebay_listings`, `slack_webhook_ebay_markdown`). No literals in any script as of 2026-09-05. NEVER `exec()`/import an `~/ebay_*.py` to "verify" it — they run live writes at module level; use `py_compile`.
...
- **Store subscription tier:** all 5 stores subscribed (insertion fees ≈ $0; `GetStore` does not return the tier). Superseded: the 2026-06-29 "Roanoke has no Store" finding.
- **Payments:** eBay Managed Payments. *(CONFIRM the linked bank account on file.)*
- **eBay developer API:** Trading API (Auth'n'Auth tokens, all 5 stores) is the working path. REST Sell APIs (`sell.analytics` seller standards, `sell.marketing` Promoted Listings, `sell.finances`, `sell.negotiation` offers-to-watchers) need OAuth consent per store — only Lexington done (2026-08-24, 2-hour token, no refresh token). `GetSellerDashboard` is retired (404); `findCompletedItems`/Marketplace Insights are closed — sold comps come from Terapeak, never the API (proven 2026-08-14, don't re-litigate).
