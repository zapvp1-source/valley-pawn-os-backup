#!/usr/bin/env python3
# Analysis pass over raw_pull.json -> summary.json + summary.md (2026-09-07 run)
...
for name in STORES:
    ...
    def recat(type_str):
        t = (type_str or '').lower()
        # 2026-08-31 confirmed: eBay reports FVF under the generic AccountDetailsEntryType 'CustomCode'.
        # 2026-09-07: type strings shifted to short codes (FeeInsertion/FeeAd/FeeReturnShipping/CustomCode)
        # that don't match the original substring keywords -- map them explicitly instead of guessing.
        if t == 'customcode': return 'final_value_fee'
        if t == 'feead': return 'promoted_listings'
        if t == 'feeinsertion': return 'insertion'
        if t == 'feereturnshipping': return 'return_shipping'
        if 'final value' in t or 'fvf' in t: return 'final_value_fee'
        if 'promoted' in t or 'ad fee' in t: return 'promoted_listings'
        if 'insertion' in t: return 'insertion'
        if 'international' in t: return 'international'
        if 'return' in t: return 'return_shipping'
        if 'subscription' in t or 'store' in t: return 'subscription'
        return 'other'

    for f in fees90:
        cat = recat(f.get('type'))
        amt = f.get('amt') or 0
...
# ---- Prior run comparison (2026-08-31 full run) ----
prior_path = os.path.join(os.path.dirname(BASE), '2026-08-31', 'summary.json')
prior = json.load(open(prior_path))

deltas = {'vs_2026-08-31': {}}
for name in STORES:
    p = prior['stores'][name]
    c = summary['stores'][name]
    deltas['vs_2026-08-31'][name] = {
        'active_delta': c['active_count'] - p['active_count'],
        'listed_value_delta': round(c['listed_value'] - p['listed_value'], 2),
        'sold90_revenue_delta': round(c['sold90_revenue'] - p['sold90_revenue'], 2),
        'fees90_total_delta': round(c['fees90_total'] - p['fees90_total'], 2),
        'aged_90plus_count_delta': c['aged_90plus_count'] - p['aged_90plus_count'],
        'aged_90plus_value_delta': round(c['aged_90plus_value'] - p['aged_90plus_value'], 2),
        'unread_delta': c['messages_unread_60d'] - p['messages_unread_60d'],
        'neg_no_resp_delta': c['neg_neutral_no_response_count'] - p['neg_neutral_no_response_count'],
        'open_best_offers_delta': c['open_best_offers'] - p['open_best_offers'],
    }
pc = prior['channel']; cc = summary['channel']
deltas['vs_2026-08-31']['channel'] = {
    'active_delta': cc['active_count'] - pc['active_count'],
    'listed_value_delta': round(cc['listed_value'] - pc['listed_value'], 2),
    'sold90_revenue_delta': round(cc['sold90_revenue'] - pc['sold90_revenue'], 2),
    'fees90_total_delta': round(cc['fees90_total'] - pc['fees90_total'], 2),
    'fee_pct_delta': round((cc['fee_pct_of_revenue'] or 0) - (pc['fee_pct_of_revenue'] or 0), 1),
    'aged_90plus_count_delta': cc['aged_90plus_count'] - pc['aged_90plus_count'],
    'aged_90plus_value_delta': round(cc['aged_90plus_value'] - pc['aged_90plus_value'], 2),
    'unread_delta': cc['messages_unread_60d'] - pc['messages_unread_60d'],
    'neg_no_resp_delta': cc['neg_neutral_no_response_count'] - pc['neg_neutral_no_response_count'],
    'open_best_offers_delta': cc['open_best_offers'] - pc['open_best_offers'],
    'trs_eligible_delta': cc['trs_eligible_count'] - pc['trs_eligible_count'],
    'markdown_capped_delta': cc['markdown_capped_active_no_action_count'] - pc['markdown_capped_active_no_action_count'],
}
...
print('=== CHANNEL ===')
print(json.dumps(summary['channel'], indent=1)[:2000])
print('=== DELTAS vs 8/31 ===')
print(json.dumps(deltas['vs_2026-08-31']['channel'], indent=1))
print('=== PER STORE ===')
for name in STORES:
    c = summary['stores'][name]
    d = deltas['vs_2026-08-31'][name]
    print(name, '| active', c['active_count'], '(', d['active_delta'], ')',
          '| listed_value', c['listed_value'], '| sold90_rev', c['sold90_revenue'],
          '| fees90', c['fees90_total'], '| fee%', c['fee_pct_of_revenue'],
          '| aged90+', c['aged_90plus_count'], '(', d['aged_90plus_count_delta'], ')',
          '| trs_elig', c['trs_eligible_count'], '/', c['active_count'],
          '| unread', c['messages_unread_60d'], '| neg_no_resp', c['neg_neutral_no_response_count'],
          '| markdown_capped', c['markdown_capped_count'], '| open_BO', c['open_best_offers'], '/', c['best_offers_expiring_48h'],'exp48h')
print('DONE_ANALYZE')
