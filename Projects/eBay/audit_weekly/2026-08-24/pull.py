#!/usr/bin/env python3
# Weekly eBay channel audit pull - READ ONLY. 2026-08-24
import json, os, time
...
def parse_item(it):
    ...
    bo_en = T(bo, 'BestOfferEnabled') or g(q('BestOfferEnabled'))
    ...
    return {
        ...
        'best_offer': bo_en, 'condition': g(q('ConditionDisplayName')),
    ...
    for c in range(0, days_back, 30):
        ...
        while True:
            ...
            for en in r.findall('.//' + q('AccountEntry')):
                ents.append({'type': T(en,'AccountDetailsEntryType'), 'amt': T(en,'GrossDetailAmount')})
            pr = r.find('.//' + q('PaginationResult'))
            tot = int(T(pr,'TotalNumberOfPages','1') or 1)
            if page >= tot: break
            page += 1
...
def store_info(store):
    try:
        ...
        return {'subscription': T(r.find(q('Store')), 'Subscription') if r.find(q('Store')) is not None else None}
    except Exception as e:
...
def messages_feedback(store):
    ...
    try:
        ...
        for m in r.findall('.//' + q('Message')):
            rec['messages'].append({'read': T(m,'Read'), 'folder': T(m.find(q('Folder')), 'FolderID'), 'subject': T(m,'Subject')})
    except Exception as e:
        rec['err'].append('msg:'+str(e)[:150])
    try:
        r = call(store['token'], 'GetFeedback', '<DetailLevel>ReturnAll</DetailLevel><Pagination><EntriesPerPage>100</EntriesPerPage><PageNumber>1</PageNumber></Pagination>')
        rec['feedback'] = {'score': T(r,'FeedbackScore')}
        det=[]
        for f in r.findall('.//' + q('FeedbackDetail')):
            if T(f,'CommentType') in ('Negative','Neutral'):
                det.append({'type':T(f,'CommentType'),'text':T(f,'CommentText'),'date':T(f,'CommentTime'),'item':T(f,'ItemID'),'response':T(f,'Response')})
        rec['neg_neutral']=det
    except Exception as e:
    ...
    try:
        ...
        for arr in r.findall('.//' + q('BestOfferArray')):
            for bo in arr.findall(q('BestOffer')):
                out['offers'].append({'id':T(bo,'BestOfferID'),'status':T(bo,'Status'),'expire':T(bo,'ExpirationTime'),'item':T(bo,'ItemID')})
    except Exception as e:
        out['err'].append(str(e)[:150])
    return out
...
for s in STORES:
    ...
    try:
        ...
        rec['active']=[]; rec['active_error']=str(e)[:200]
    print('  active', len(rec.get('active',[])), flush=True)
    try:
        rec['sold90'] = sold(s, 90)
    except Exception as e:
        rec['sold90']=[]; rec['sold_error']=str(e)[:200]
    print('  sold90', len(rec.get('sold90',[])), flush=True)
    try:
        rec['fees90'] = fees(s, 90)
    except Exception as e:
        rec['fees90']=[]; rec['fees_error']=str(e)[:200]
    print('  fees90', len(rec.get('fees90',[])), flush=True)
    try:
        rec['store_info'] = store_info(s)
    except Exception as e:
        rec['store_info']={'error':str(e)[:150]}
    try:
        rec['msgfb'] = messages_feedback(s)
    except Exception as e:
        rec['msgfb']={'error':str(e)[:150]}
    try:
        rec['offers'] = offers(s)
    except Exception as e:
        rec['offers']={'error':str(e)[:150]}
    result[name] = rec
