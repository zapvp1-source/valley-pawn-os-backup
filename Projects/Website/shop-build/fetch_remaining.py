...
def main():
    ...
    db = json.load(open(items_path))
    seen = set(x["u"] for x in db["items"])
    counts = {}
    for name, slug in STORES:
        ...
        for pgn in range(1, 9):
            ...
            if len(html_txt) < 50000:
                ...
                html_txt = fetch(url, path)
                if len(html_txt) < 50000:
                    print("WARN2 %s pg%d short=%d giving up page" % (name, pgn, len(html_txt)), flush=True)
                    break
        ...
        counts[name] = got
        time.sleep(10)
    json.dump(db, open(items_path, "w"))
    print("DONE counts=%s total=%d" % (counts, len(db["items"])), flush=True)
