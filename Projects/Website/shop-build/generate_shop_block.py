...
.vp-shop-app .vp-h1{text-align:center;font-size:clamp(24px,3.4vw,34px);line-height:1.15;margin:6px 16px 2px;letter-spacing:-.01em;color:var(--ink)}
...
# --- VP-SEO-PATCH 2026-08-22: H1 + ItemList JSON-LD (audit remediation) -------
import json as _json
def _seo_ld(items, limit=120):
    """Top-N by price -> schema.org ItemList of Products. Keeps page weight sane."""
    def _p(x):
        try: return float(str(x.get('p','0')).replace('$','').replace(',',''))
        except Exception: return 0.0
    top = sorted(items, key=_p, reverse=True)[:limit]
    els = []
    for n, i in enumerate(top, 1):
        els.append({
            ...
            "item": {
                ...
                "name": i.get('t',''),
                "image": i.get('img',''),
                "brand": {"@type": "Brand", "name": "Valley Pawn"},
                "offers": {
                    "@type": "Offer",
                    "price": "%.2f" % _p(i),
                    "priceCurrency": "USD",
                    ...
                    "url": i.get('u',''),
                    "seller": {"@type": "Organization", "name": "Valley Pawn (%s)" % i.get('s','')}
                }
            }
        })
    doc = {"@context": "https://schema.org", "@type": "ItemList",
           "name": "Valley Pawn live inventory",
           "numberOfItems": len(els), "itemListElement": els}
    return ('<script type="application/ld+json">'
            + _json.dumps(doc, separators=(',', ':')) + '</script>')

SEO_LD = _seo_ld(ITEMS)
SEO_H1 = ('<h1 class="vp-h1">Shop Valley Pawn \u2014 Live Inventory from All 5 '
          'Virginia Stores</h1>')
# --- end VP-SEO-PATCH ---------------------------------------------------------
