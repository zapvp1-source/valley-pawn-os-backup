...
BASE = "/sessions/gifted-hopeful-noether/mnt/Website/shop-build"
...
def main():
    # warm up cookie jar with a homepage hit
    ...
    for name, slug in STORES:
        ...
        for pgn in range(1, 9):
            ...
            if len(html_txt) < 50000:
                print("WARN %s pg%d short=%d -- retrying once after pause" % (name, pgn, len(html_txt)), flush=True)
                time.sleep(6)
                html_txt = fetch(url, path, referer)
                if len(html_txt) < 50000:
                    print("WARN %s pg%d still short=%d -- giving up on this page" % (name, pgn, len(html_txt)), flush=True)
                    break
