...
BASE = '/sessions/lucid-peaceful-clarke/mnt/Website/shop-build'
WORK = '/sessions/lucid-peaceful-clarke/mnt/outputs/shop-run'
import os as _os
_os.makedirs(WORK, exist_ok=True)
RAW  = os.path.join(WORK, 'raw_now')
CJ   = os.path.join(WORK, 'vp_ebay_cookiejar_now.txt')
...
def main():
    fetch('https://www.ebay.com/', os.path.join(RAW, '_warmup.html'), 'https://www.google.com/')
    time.sleep(2)
    ...
    for name, slug in STORES:
        ...
        for pgn in range(1, 9):
            ...
            if len(html_txt) < 50000:
                print('WARN %s pg%d short=%d -- retrying once' % (name, pgn, len(html_txt)), flush=True)
                time.sleep(6)
                html_txt = fetch(url, path, referer)
                if len(html_txt) < 50000:
                    print('WARN %s pg%d still short=%d -- giving up' % (name, pgn, len(html_txt)), flush=True)
                    break
    ...
    with open(os.path.join(WORK, 'items_now.json'), 'w') as f:
        json.dump(db, f)
