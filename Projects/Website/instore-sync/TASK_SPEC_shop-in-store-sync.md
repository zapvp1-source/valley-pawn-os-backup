# shop-in-store-sync — full task spec (staged 2026-08-21)

Registration target: LOCAL Cowork scheduled task, taskId `shop-in-store-sync`, cron `10 10,16 * * *` (10:10 AM + 4:10 PM daily, local), model pin: sonnet.

If this file is being read by a future session: the spec below is the complete, ready-to-register prompt. Register it verbatim via create_scheduled_task, then verify the first run against the live page (Rule 12), not the run record.
