...
description: Last-day-of-month 8 PM — launch the native prestage runner (bin/monthly_prestage_runner.py) that stages the 6 EOM date-window XLSX files monthly-analytics-report reads at 1:45 AM on the 1st. Claude only starts and verifies the runner; the runner does the serial drop → wait → copy loop itself. Silent on failure.
