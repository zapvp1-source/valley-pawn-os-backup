...
description: 1st of each month, 2 AM ET — FINAL prior-month employee sales rankings (Retail Sales Excluding Fees, the same metric as the weekly MTD post) from a fresh full-month Bravo employee-activity pull via the pipeline trigger queue. Posts to #employee-performance and saves the monthly workbook. Rebuilt 2026-09-05 — pipeline-sourced, zero computer-use.
